...
Todo contenedor que agrupe dos o más `[data-card="true"]` como hermanos directos debe conservar una separación visible de al menos `sm` (8px). La excepción canónica es `.cd-card`: usa literalmente `margin: 10px 5px 10px 5px;` para dar la separación consistente solicitada. Cuando haya dos o más `.cd-card` hermanas, su contenedor debe tener `gap: 0` para no sumar espaciado; para el resto de las cards, el espacio corresponde al contenedor mediante un token de spacing. `npm run test:padding-audit` verifica tanto el mínimo como que no se duplique con un `gap` de contenedor.

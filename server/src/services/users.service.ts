...
export type CreateOrganizationUserInput = {
  ...
  functionId?: string | null; campaignId?: string | null;
};
...
function normalize(input: CreateOrganizationUserInput) {
  ...
  return { firstName, lastName, displayName: `${firstName} ${lastName}`, phone: input.phone?.trim() ?? '', email, password, campaignId: input.campaignId?.trim() || null, functionId: input.functionId?.trim() || null };
}
...
async function uploadAvatar(uid: string, avatar?: Express.Multer.File) {
  ...
  if (avatar.size > 5 * 1024 * 1024) throw new ValidationError('La foto de perfil no puede superar 5 MB.');
...
export async function createOrganizationUser(user: DecodedIdToken, orgId: string, input: CreateOrganizationUserInput, avatar?: Express.Multer.File) {
  ...
  } else if (data.functionId) throw new ValidationError('Elegí una campaña para asignar una función.');
  ...
  try {
    ...
    if (data.campaignId) {
      ...
      batch.set(memberRef, { email: data.email, displayName: data.displayName, role: 'usuario', functionId: data.functionId, teamId: null, joinedAt: FieldValue.serverTimestamp() });
    }
    await batch.commit();
    await adminAuth.setCustomUserClaims(created.uid, { role: 'usuario', orgId, camps: data.campaignId ? { [data.campaignId]: true } : {} });
    return { uid: created.uid, email: data.email, displayName: data.displayName, firstName: data.firstName, lastName: data.lastName, phone: data.phone, photoURL, role: 'usuario', campaignId: data.campaignId, functionId: data.functionId };
  } catch (error) {

...
try {
  ...
  if ((await campaign.collection('treoBudgetLines').doc(budget.id).get()).data().paid !== 0) throw new Error('Enviado se contó como pagado.'); await post(`${base}/payments/${payment.id}/confirm`, {}); await post(`${base}/payments/${payment.id}/reconcile`, {});
  ...
  const advance = await post(`${base}/advances`, { responsibleId: user.uid, purpose: 'Viáticos móvil', activity: 'Recorrida', amount: 200000, fundingAccountId: bank.id }); const settled = await post(`${base}/advances/${advance.id}/settle`, { approvedExpenses: 160000, returned: 40000, expenseAccountId: '5-1-03', advanceAccountId: '1-1-04', receiptDocumentIds: [] });

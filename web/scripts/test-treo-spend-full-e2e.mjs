...
try {
  ...
  const budget = await post(`${base}/budgets`, { name: `Imprenta ${suffix}`, approved: 100000, stage: 'Campaña', activity: 'Volantes', costCenter: 'Territorio' }); if (budget.version !== 1) throw new Error('La partida no inició su versionado.');
  const expansion = await post(`${base}/budget-changes`, { kind: 'ampliacion', toBudgetId: budget.id, amount: 20000, reason: 'Refuerzo de tirada' }); await post(`${base}/budget-changes/${expansion.id}/approve`, {}); const expanded = await campaign.collection('treoBudgetLines').doc(budget.id).get(); if (expanded.data().approved !== 120000 || expanded.data().version !== 2) throw new Error('La ampliación aprobada no versionó la partida.');
  const order = await post(`${base}/orders`, { budgetId: budget.id, vendorId: legacyVendor.id, object: 'Imprenta', amount: 100000, orderedQty: 100, operationId: `${suffix}:order` }); const committed = await campaign.collection('treoBudgetLines').doc(budget.id).get(); if (order.amount !== 100000 || committed.data().committed !== 100000 || committed.data().approved - committed.data().committed - committed.data().recognized !== 20000) throw new Error('La ampliación no dejó disponibilidad correcta para el compromiso.');
  ...
  const receiptForm = new FormData(); receiptForm.append('kind', 'rendicion'); receiptForm.append('operationId', advance.operationId); receiptForm.append('document', new Blob([Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScL2mQAAAABJRU5ErkJggg==', 'base64')], { type: 'image/png' }), 'rendicion-movil.png');

...
try {
  ...
  if (!stored.exists || !stored.data()?.startAt || !stored.data()?.endAt || stored.data()?.revision !== 1 || stored.data()?.type !== customType) throw new Error('El evento operativo no se persistió con start/end/revision/tipo personalizado en Firestore.');

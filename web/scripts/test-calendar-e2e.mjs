...
try {
  ...
  const created = []; const createdTasks = []; const createdResources = []; const createdTemplates = [];
  cleanup = async () => { await Promise.all(created.map((id) => campaignRef.collection('calendar').doc(id).delete())); await Promise.all(createdTasks.map((id) => campaignRef.collection('tasks').doc(id).delete())); await Promise.all(createdResources.map((id) => campaignRef.collection('calendarResources').doc(id).delete())); await Promise.all(createdTemplates.map((id) => campaignRef.collection('calendarTemplates').doc(id).delete())); };
  ...
  const title = `E2E calendario ${Date.now()}`; const customType = `Operativo E2E ${Date.now()}`;
  await page.getByRole('button', { name: /Crear evento/ }).click(); const eventDialog = page.getByRole('dialog'); await eventDialog.waitFor(); await eventDialog.getByLabel('Título').fill(title); await eventDialog.getByLabel('Tipo').fill(customType);
  const publicationToggle = eventDialog.locator('label.form-check').filter({ hasText: 'Publicación' }).locator('input[type="checkbox"]'); await publicationToggle.check(); await eventDialog.getByText('Permitir edición a usuarios vinculados', { exact: true }).waitFor();
  const firstLinkedMember = eventDialog.locator('.electoral-calendar__modal-section').first().locator('.electoral-calendar__checkbox-grid input[type="checkbox"]').first(); if (await firstLinkedMember.count()) await firstLinkedMember.check();
  ...
  const taskId = `calendar-publication-${createdEvent.eventId}`; createdTasks.push(taskId);
  if (!stored.exists || !stored.data()?.startAt || !stored.data()?.endAt || stored.data()?.revision !== 1 || stored.data()?.type !== customType || stored.data()?.isPublication !== true || !(await campaignRef.collection('tasks').doc(taskId).get()).exists) throw new Error('El evento de publicación o su tarea espejo no se persistieron con los datos esperados en Firestore.');
  await page.getByText(title, { exact: false }).first().click(); await page.getByRole('heading', { name: title, exact: true }).waitFor(); await page.getByText('Exportar ICS', { exact: true }).waitFor();
  ...
  console.log('CALENDAR E2E: agenda, vistas, recursos, plantillas, disponibilidad, creación, edición, exportación y tipo personalizado persistido en Firestore OK.');

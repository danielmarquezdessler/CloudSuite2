  ...
  return <>
    {menuItems.map((item: MenuItem) => {
      ...
      if (item.addon) {
        ...
        if (enabled) return <li key={item.id} className="pc-item cloudsuite-sidebar-addon is-enabled"><Link to={item.link || '/'} className="pc-link cloudsuite-sidebar-addon__button"><span className="pc-micon"><i className={item.icon} /></span><span className="pc-mtext">{t(item.label)}</span></Link></li>;

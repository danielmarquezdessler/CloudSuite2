...
export function Toast({ message, variant = 'success', onClose }: { message: ReactNode; variant?: 'success' | 'danger' | 'info'; onClose: () => void }) {
  ...
  return <div className={`cd-toast alert alert-${variant} shadow position-fixed top-0 end-0 m-4`} role="status" style={{ maxWidth: 360 }}>{message}<button className="btn-close float-end" aria-label="Cerrar" onClick={onClose} /></div>;
}

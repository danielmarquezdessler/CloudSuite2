...
export default function PageContainer({ children, className = '', backTo, backLabel }: PageContainerProps) {
  ...
  return <div className={`cs-page-container cs-page ${className}`.trim()}><div className="cs-page-container__content">{children}</div>{resolvedBackTo && <div className="cs-page-container__back" data-back-button="page"><BackButton to={resolvedBackTo} label={backLabel ?? 'Regresar a inicio'} variant="link" /></div>}{location.pathname.startsWith('/smartplanner') && <CommunicationsWidget />}</div>;
}

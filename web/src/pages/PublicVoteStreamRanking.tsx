...
type PublicRanking = { name: string; location: string; date: string; candidates: Array<{ id: string; name: string; party: string; photoUrl?: string | null; partyLogoUrl?: string | null; votes: number }> };
export default function PublicVoteStreamRanking() {
  ...
  return <main className="vote-public-ranking"><header><span>VOTE STREAM</span><h1>{ranking?.name ?? 'Cargando ranking…'}</h1><p>{ranking?.location} · {ranking?.date}</p></header><section aria-live="polite">{rows.map((candidate, index) => <article key={candidate.id} className="vote-public-ranking__row"><span>#{index + 1}</span>{candidate.photoUrl ? <img src={candidate.photoUrl} alt="" /> : <span className="vote-public-ranking__avatar">{candidate.name.slice(0, 2)}</span>}<div><strong>{candidate.name}</strong><small>{candidate.party}</small></div><b>{total ? Math.round(candidate.votes / total * 100) : 0}%</b><em>{candidate.votes} votos</em></article>)}</section><footer>Los datos publicados no son datos oficiales, son informaciones enviadas por el equipo de campaña. Compartir estos datos se encuentra sujeto a la legislación electoral de su país.</footer></main>;
}

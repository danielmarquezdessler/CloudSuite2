...
export type ManualVoterInput = { name: string; phone: string; email: string; address: string; lat: number | null; lng: number | null; confirmSimilar?: boolean };
type ExistingVoter = { id: string; name: string; address?: string };
type Props = { show: boolean; onHide: () => void; onCreate: (values: ManualVoterInput) => Promise<void>; existingVoters: ExistingVoter[] };

const normalize = (value: string) => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/\s+/g, ' ').trim();
const blank = (): ManualVoterInput => ({ name: '', phone: '', email: '', address: '', lat: null, lng: null });

export default function CreateVoterModal({ show, onHide, onCreate, existingVoters }: Props) {
  ...
  const submit = async (confirmed = false) => {
    ...
    const found = existingVoters.find((voter) => normalize(voter.name) === normalize(values.name) && normalize(voter.address ?? '') === normalize(values.address));
    if (found && !confirmed) { setSimilar(found); return; }
    setSaving(true);
    try { await onCreate({ ...values, confirmSimilar: confirmed }); setValues(blank()); setSimilar(null); onHide(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'No pudimos crear el elector.'); } finally { setSaving(false); }
  };
  ...
  return <Modal show={show} onHide={close} centered enforceFocus={false} aria-labelledby="create-voter-title">
    <Modal.Header closeButton><Modal.Title id="create-voter-title">Crear elector</Modal.Title></Modal.Header>
    ...
    <Modal.Footer><Button variant="secondary" onClick={close} disabled={saving}>Cancelar</Button>{!similar && <Button onClick={() => void submit()} disabled={saving}>{saving ? 'Guardando…' : 'Crear elector'}</Button>}</Modal.Footer>
  </Modal>;
}

...
export default function Tickets() {
  ...
  return <PageContainer><Stack gap="lg"><HeroBanner icon="doc" eyebrow="SOPORTE INTERNO" title="Tickets" subtitle="Ordená solicitudes y seguimiento entre áreas." ctaLabel="Nuevo ticket" onCtaClick={() => setShowCreate(true)} />
    <ContentPanel icon="list" title="Solicitudes" subtitle="Filtrá y actualizá cada pedido del cuartel."><Stack gap="sm"><Inline gap="sm"><select aria-label="Filtrar por estado" className="form-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Todos los estados</option><option value="abierto">Abiertos</option><option value="en_progreso">En progreso</option><option value="resuelto">Resueltos</option></select><select aria-label="Filtrar por área" className="form-select" value={areaFilter} onChange={(event) => setAreaFilter(event.target.value)}><option value="todos">Todas las áreas</option>{(areasQ.data ?? []).map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></Inline>{rows.map((ticket) => <article className="sp-project" key={ticket.id}><Inline gap="sm" className="sp-project__row"><Stack gap="xs"><strong>{ticket.title}</strong><small>{ticket.description || 'Sin descripción'} · {ticket.status}</small></Stack><Inline gap="xs" wrap><button type="button" className="btn btn-outline-primary" onClick={() => void move(ticket, 'en_progreso')}>En progreso</button><button type="button" className="btn btn-outline-success" onClick={() => void move(ticket, 'resuelto')}>Resolver</button><button type="button" className="btn btn-outline-danger" onClick={() => void remove(ticket)}>Eliminar</button></Inline></Inline></article>)}{!rows.length && <small className="text-muted">No hay tickets para los filtros elegidos.</small>}{notice && <div className="alert alert-info mb-0" role="status">{notice}</div>}</Stack></ContentPanel>
    <Modal show={showCreate} onHide={() => setShowCreate(false)}><form onSubmit={save}><Modal.Header closeButton><Modal.Title>Nuevo ticket</Modal.Title></Modal.Header><Modal.Body><Stack gap="md"><input aria-label="Título de ticket" className="form-control" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Título de la solicitud" required /><textarea aria-label="Descripción de ticket" className="form-control" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Contexto y detalle" /><select aria-label="Área asignada" className="form-select" value={form.assignedArea} onChange={(event) => setForm({ ...form, assignedArea: event.target.value })} required><option value="">Área asignada</option>{(areasQ.data ?? []).map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select><select aria-label="Prioridad del ticket" className="form-select" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })}><option value="baja">Prioridad baja</option><option value="media">Prioridad media</option><option value="alta">Prioridad alta</option></select></Stack></Modal.Body><Modal.Footer><PrimaryButton type="submit" icon="plus">Crear ticket</PrimaryButton></Modal.Footer></form></Modal>
  </Stack></PageContainer>;
}

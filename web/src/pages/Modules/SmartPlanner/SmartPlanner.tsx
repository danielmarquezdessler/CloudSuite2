...
export default function SmartPlanner() {
  ...
  const configQ = useAuthenticatedQuery<{ accent?: string }>(user, base ? `${base}/config` : null, [base]);
  ...
  return <PageContainer><div data-smartplanner="workspace" style={{ '--smartplanner-accent': configQ.data?.accent || '#0060F0' } as CSSProperties}>
    ...
    <ContentPanel icon="clock" title="Jornada electoral" subtitle="Acceso directo al operativo crítico del Día D." headerAction={<Link className="btn btn-danger" to="/smartplanner/election-day">Ver Panel del Día D</Link>}><Inline gap="sm" wrap><Link className="btn btn-outline-danger" to="/smartplanner/war-room">Abrir War Room</Link><Link className="btn btn-outline-primary" to="/smartplanner/promises">Revisar propuestas</Link><Link className="btn btn-outline-primary" to="/smartplanner/tickets">Tickets internos</Link><Link className="btn btn-outline-primary" to="/smartplanner/chat">Chat por área</Link><Link className="btn btn-outline-primary" to="/smartplanner/reports">Reportes</Link><Link className="btn btn-outline-primary" to="/smartplanner/staff">Personal</Link><Link className="btn btn-outline-primary" to="/smartplanner/settings">Configuración</Link></Inline></ContentPanel>
    <ContentPanel icon="target" title="Cuartel de campaña" subtitle="Elegí un frente para entrar a su tablero operativo." headerAction={<Inline gap="sm"><Link className="sp-tab" to="/smartplanner/contributors">Aportantes</Link><Link className="sp-tab" to="/smartplanner/providers">Proveedores</Link><Link className="sp-tab" to="/smartplanner/invoices">Facturación</Link><Link className="sp-tab" to="/smartplanner/contracts">Contratos</Link><Link className="sp-tab" to="/smartplanner/materials">Materiales</Link><Link className="sp-tab" to="/smartplanner/operations">Avanzada</Link><Link className="sp-tab" to="/smartplanner/crews">Cuadrillas</Link><Link className="sp-tab" to="/smartplanner/messages">Mensajes</Link><button type="button" className={`sp-tab ${view === 'kanban' ? 'is-active' : ''}`} onClick={() => setView('kanban')}>Kanban</button><button type="button" className={`sp-tab ${view === 'gantt' ? 'is-active' : ''}`} onClick={() => setView('gantt')}>Gantt</button></Inline>}>
      <Stack gap="lg">
    ...
    <AreaChat embedded areaId={selectedAreaId} />
